...
class ISBNModal_ extends Component {
    ...
    handleRegisterISBN(e) {
        ...
        if (this.state.isbn.length !== 13 && this.state.isbn.length !== 10) {
            return this.setState({
                isInvalid: true,
                invalidMessage: ISBNModal.INVALID_ISBN_LENGTH,
            });
        }
        ...
        storeISBNToUserBookDirect(user.id, this.state.isbn)
            .then(res => {
                if (res.status === 401) {
    ...
    render() {
        ...
        if (!getAuthUser()) {
            return (
                <button type="button" className="btn btn-success" onClick={this.setAlert}>
                    {btnText}
                </button>
            );
        }
