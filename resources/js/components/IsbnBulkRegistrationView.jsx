...
class IsbnBulkRegistrationView extends Component {
    ...
    handleSubmitRegister(e) {
        ...
        storeIsbnBulkRegisterDirect(userId, { isbnList })
            .then(res => {
                if (res.ok) {
                    return res.json();
                }
                ...
                if (res.status == 400) {
                    this.setState({
                        invalidMessage:
                            '空行含まれているか、ISBNの書式が間違っている可能性があります。ご確認ください。',
                    });
                } else {
                    this.setState({
                        invalidMessage:
                            res.statusText +
                            ': 登録時にエラーが発生しました。恐れ入りますがフッターの「ご意見」からご連絡ください。',
                    });
                }
                throw new Error();
            })
            ...
            <div className="container mt-4">
                <div className="row justify-content-center">
                    <div className="main-content">
                        <BackButtonArea
                            to={`/users/${this.props.loggedinUser.id}/user_books`}
                            text="本棚へ"
                        />
                        <div className="mb-5">
                            <h1>まとめて登録(ISBN)</h1>
                            <p>
                                本に記載されているISBNを入力することで、簡単に本を自分の本棚に一括登録することができます。
                                <br />
                                一括登録には若干の時間がかかる場合がありますが、不具合ではありません。
                            </p>
                        </div>

                        <form onSubmit={this.handleSubmitRegister}>
                            <div className="form-group">
                                <label htmlFor="text">ISBN(複数行)</label>
